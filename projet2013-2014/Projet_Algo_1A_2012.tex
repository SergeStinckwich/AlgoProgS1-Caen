...
On se propose de réaliser une variante du jeu {\bf Ricochets Robots} : \url{https://fr.wikipedia.org/wiki/Ricochet_Robots}.
Il s'agit d'un jeu de plateau qui peut se jouer seul ou à plusieurs.
...
est de déplacer chacun à leur tour les 4 robots afin d'atteindre la cible en s'aidant éventuellement des autres robots. Voir des exemples de grilles de jeu ici :
\url{http://jeux.prise2tete.fr/p2t-ricochet/p2t-ricochet.php5}.

Seul sont autorisés des versions textuelles de ce jeu.
Le programme devra permettre de visualiser la partie et de gérer un
système de high-scores.

Les grilles seront construites de manière aléatoire ou bien éditée par
un utilisateur. Un mode joueur aléatoire pourra être implémenté.

La version complète du jeu décrite ici :
\url{https://fr.wikipedia.org/wiki/Ricochet_Robots} pourra être réalisée dans un deuxième temps.

Votre programme devra permettre de visualiser une séquence de jeu en affichant à chaque tour : le plateau de jeu, qui doit jouer (dans le cas de la version à plusieurs joueurs), le score des joueurs.
...
Pour les besoins du jeu, il pourra être utile de définir un joueur aléatoire qui joue aléatoirement.
